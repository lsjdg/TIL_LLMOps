...
WITH TEMP_POPLTN_SE_CNT AS(
    ...
    SELECT
        ADMINIST_ZONE_NO,
        ADMINIST_ZONE_NM,
        MALE_POPLTN_CNT / (MALE_POPLTN_CNT + FEMALE_POPLTN_CNT) AS MALE_POPLTN_RATIO,
        FEMALE_POPLTN_CNT / (MALE_POPLTN_CNT + FEMALE_POPLTN_CNT) AS FEMALE_POPLTN_RATIO
    FROM TEMP_POPLTN_SE_CNT_PIVOT
), TEMP_MALE_POPLTN_SE_RATIO_RANK AS(
    SELECT
        ADMINIST_ZONE_NO,
        ADMINIST_ZONE_NM,
        RANK() OVER(ORDER BY MALE_POPLTN_RATIO DESC) AS MALE_POPLTN_RATIO_RANK
    FROM TEMP_POPLTN_SE_RATIO
),TEMP_FEMALE_POPLTN_SE_RATIO_RANK AS(
    SELECT
        ADMINIST_ZONE_NO,
        ADMINIST_ZONE_NM,
        RANK() OVER(ORDER BY FEMALE_POPLTN_RATIO DESC) AS FEMALE_POPLTN_RATIO_RANK
    FROM TEMP_POPLTN_SE_RATIO
), TEMP_MALE_MIN_RANK AS(
    SELECT
        `ADMINIST_ZONE_NO`,
        `ADMINIST_ZONE_NM`,
        `MALE_POPLTN_RATIO_RANK`
    FROM TEMP_MALE_POPLTN_SE_RATIO_RANK
    WHERE MALE_POPLTN_RATIO_RANK = 1
), TEMP_MALE_MAX_RANK AS(
    SELECT
        `ADMINIST_ZONE_NO`,
        `ADMINIST_ZONE_NM`,
        MAX(`MALE_POPLTN_RATIO_RANK`) AS MALE_POPLTN_RATIO_RANK
    FROM TEMP_MALE_POPLTN_SE_RATIO_RANK
)
-- SELECT * FROM TEMP_MALE_MIN_RANK;
-- UNION ALL
SELECT * FROM TEMP_MALE_MAX_RANK;
